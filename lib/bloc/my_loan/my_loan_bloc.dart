...
class MyLoanBloc extends Bloc<MyLoanEvent, MyLoanState> {
  ...
  void _mapGetRecommendedEventToState(event, Emitter<MyLoanState> emit) async {
    ...
    try {
      ...
      myLoans = (json.decode(response.body)['myloan'] as List)
          .map((i) => MyLoanInfo.fromJson(i))
          .toList();

      var pendingCount =
          myLoans.where((i) => i.status == 'pending').length ?? 0;
      var approvedCount =
          myLoans.where((i) => i.status == 'approved').length ?? 0;
      var closedCount = myLoans.where((i) => i.status == 'Closed').length ?? 0;
      var declinedCount =
          myLoans.where((i) => i.status == 'declined').length ?? 0;
      ...
      emit(state.copyWith(
          status: MyLoanStatus.error, errorMessage: e.toString()));
    }
  }
}
